...
import { Car, Plus, MoreVertical, Fuel, TrendingUp } from "lucide-react";
...
export function CarCard({
  ...
  if (isAddCard) {
    return (
      ...
      >
        <Card className="glass-card border-dashed border-primary/30 h-full hover:border-primary/50 transition-all duration-300">
          <CardContent className="p-6 flex flex-col items-center justify-center h-full min-h-[200px]">
            ...
            <p className="text-sm text-muted-foreground text-center mb-4">
              Start tracking fuel efficiency for your vehicle
            </p>
            <Button 
              onClick={onAddClick}
              variant="default"
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-xs text-muted-foreground">Avg km/L</span>
                </div>
          ...
          {stats && (
            <div className="mt-3 pt-3 border-t border-border/20">
              <p className="text-xs text-muted-foreground">
                Last fill: {new Date(stats.last_fill_date).toLocaleDateString()}
              </p>
              <p className="text-xs text-muted-foreground">
